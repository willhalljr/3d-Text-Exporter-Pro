...
import { Settings, Box, X, Upload, ImagePlus, ChevronRight, ChevronLeft, RefreshCw, Maximize } from 'lucide-react';
      ...
      <div className="flex h-full w-full">
        ...
        <div className="w-12 border-r border-[#222] flex flex-col items-center py-4 gap-6 shrink-0 bg-[#0a0a0a]">
          ...
          <div className="flex-1 flex flex-col justify-center">
             <span className="[writing-mode:vertical-lr] text-[10px] font-black uppercase tracking-[0.5em] text-gray-700 select-none">Properties</span>
          </div>
        </div>
        ...
        <div className={`flex flex-col w-full transition-opacity duration-300 ${isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          <div className="p-5 border-b border-[#222] flex items-center justify-between bg-[#0f0f0f] shrink-0">
            <h2 className="font-black text-[10px] uppercase tracking-[0.3em] text-gray-500">Global Settings</h2>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-10 scroll-smooth">
            <section className="bg-black p-5 rounded-3xl border border-white/5 shadow-inner">
              <div className="flex items-center gap-2 text-[10px] font-black text-gray-600 uppercase tracking-widest mb-5">
                 <ImagePlus className="w-4 h-4 text-blue-500/50" /> Surface Map
              </div>
                  ...
                  <label className={`group flex flex-col items-center justify-center min-h-[120px] border-2 border-dashed rounded-2xl transition-all cursor-pointer overflow-hidden ${materialSettings.map ? 'border-blue-500 bg-blue-500/5' : 'border-[#222] bg-black hover:border-blue-500/30'}`}>
                    {materialSettings.map ? (
                      ...
                      <div className="text-center py-5">
                        ...
                        <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Upload Texture</p>
                      </div>
                    )}
            ...
            <section className="space-y-8">
              <div className="space-y-4">
                <label className="text-[10px] text-gray-600 font-black uppercase tracking-[0.2em] flex justify-between items-center">
                  Width Scale <span>{textSettings.targetLength}m</span>
                </label>
              ...
              <div className="grid grid-cols-2 gap-5">
                 <div className="space-y-3">
                    <label className="text-[10px] text-gray-600 font-black uppercase tracking-widest">Extrude</label>
                 ...
                 <div className="space-y-3">
                    <label className="text-[10px] text-gray-600 font-black uppercase tracking-widest">Finish</label>
                    <div className="pt-3">
                      <input type="range" min="0" max="1" step="0.01" value={materialSettings.metalness} onChange={(e) => updateMaterial('metalness', parseFloat(e.target.value))} className="w-full accent-blue-600 h-1.5 rounded-full bg-black appearance-none cursor-pointer" />
                    </div>
                 </div>
              </div>
            </section>
            ...
            <section className="pt-8 border-t border-[#222] space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => onExportImage(1)} className="py-3.5 bg-white/5 hover:bg-white/10 border border-[#222] rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all text-gray-400 hover:text-white flex items-center justify-center gap-2">
                  <Maximize className="w-3.5 h-3.5" /> Render
                </button>
